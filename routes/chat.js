...
const broadcast = async (user, message, room, file = null) => {
...
const sanitizeRequest = (req) => {
    ...
    if (req.body.handle && req.body.passcode) {
        ...
        if (req.body.room == "")
            req.body.room = sanitizeRoomName(DEFAULT_ROOM);
    }
...
const filesLimitHandler = (req, res, next) => {
...
router.post("*", async (req, res, next) => {
    ...
    if (req.files && req.files.fileupload) {
        if (req.files.fileupload.truncated)
            return; // File is too large
        req.file = req.files.fileupload;
    }
    ...
    if (req.url.startsWith(URL_PREFIX + ROUTES.WRITE_MESSAGE))
        isPosting = true;
    let user = getUserByToken(req.query.token) || undefined;
...
/* WRITE MSG IFRAME */
router.get(URL_PREFIX + ROUTES.WRITE_MESSAGE, (req, res, next) => {
    let user = getUserByToken(req.query.token);
    if (!user) {
        return res.render(VIEWS.ERROR, ERRORS.INVALID_TOKEN);
    }
    ...
    }, (err, html) => { user.frames.post.end(html); });
});
/* SUBMITTING WRITE MSG IFRAME */
router.post(URL_PREFIX + ROUTES.WRITE_MESSAGE, (req, res, next) => {
    ...
    try {
        ...
        user.frames.post.render(VIEWS.LAYOUT, {
            ...
            maxlen: MAX_MESSAGE_LENGTH,
            placeholder: user.nextMsgPlaceholder(req)
        }, (err, html) => { return user.frames.post.end(html); });
    }
    catch (error) {
        console.error(error);
    }
...
router.post(URL_PREFIX + ROUTES.MAIN, (req, res, next) => {
    ...
    user.tripcode.then((trip) => {
        user.tripcode = trip;
        user.frames.chatroom.render(VIEWS.LAYOUT, {
            page: VIEWS.CHATROOM,
            user: user,
            url: user.room,
...
router.get(URL_PREFIX + ROUTES.SETTINGS, (req, res, next) => {
    ...
    user.frames.settings.render(VIEWS.LAYOUT, {
        page: VIEWS.SETTINGS,
        user: user,
        theme: user.theme,
    ...
    }, (err, html) => { return user.frames.settings.end(html); });
});
...
router.get(URL_PREFIX + ROUTES.CHAT_MESSAGES, (req, res, next) => {
    ...
    user.frames.chat.render(VIEWS.LAYOUT, { page: VIEWS.VIEW_MESSAGES, user: user }, (err, html) => {
